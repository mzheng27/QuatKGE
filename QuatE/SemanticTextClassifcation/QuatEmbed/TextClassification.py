...
def main():
    ...
    EMBEDDING_MATRIX = json.load(open(os.path.join(root_path, "result/QuatE.json"), 'r'))
    #word index in matrix 
    WORD2INDEX = json.load(open(os.path.join(root_path, "QuatEmbed/word2id.json"), 'r'))

    print("start loading data")
    test_path = os.path.join(root_path, "AGNews_data/ag_news_csv/test.csv") 
    train_path = os.path.join(root_path, "AGNews_data/ag_news_csv/train.csv") 
    ...
    model = LSTM(EMBEDDING_MATRIX, input_dim=400, hidden_dim=20, layer_dim=1, output_dim=4)
    ...
    # save_path = "./param/" + "_kernelsize_" + str(kernel_size) + ".pth"
