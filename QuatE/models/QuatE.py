...
class QuatE(Model):
    ...
    def _calc(self, s_a, x_a, y_a, z_a, s_c, x_c, y_c, z_c, s_b, x_b, y_b, z_b, kernel=None):
        ...
        if kernel == None: 
            ...
            prob = kernel(A, B, C, D, s_c, x_c, y_c, z_c)

        # return -score_r                                       
        # return torch.log(1/prob - 1)                
        return prob
    ...
    def loss_nll(self, score, regul, regul2):
        #Convert batch_y from (-1,1) to (0, 1)
        y_01 = ((self.batch_y + 1) / 2).long()

        score_dim2 = torch.cat((score.reshape(-1,1), (1 - score).reshape(-1,1)), dim=1)
        
        #Change the scoring to NLLLoss
        nll_loss = nn.NLLLoss()
        return nll_loss(torch.log(score_dim2), y_01) + self.config.lmbda * regul + self.config.lmbda * regul2
    ...
    def forward(self):
        ...
        # score = self._calc(s_a, x_a, y_a, z_a, s_c, x_c, y_c, z_c, s_b, x_b, y_b, z_b, _real_value_gaussian_kernel)
        ...
        # return self.loss_nll(score, regul, regul2)
...
def _quter_inner(s_a, x_a, y_a, z_a, s_b, x_b, y_b, z_b,):
    return torch.sum(s_a * s_b + x_a * x_b + y_a * y_b + z_a * z_b, -1)

def _real_value_gaussian_kernel(s_a, x_a, y_a, z_a, s_b, x_b, y_b, z_b, Ar=0.1):
    diff = s_a - s_b, x_a - x_b, y_a - y_b, z_a - z_b
    return torch.exp(-Ar * _quter_inner(*diff, *diff))
